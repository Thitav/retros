...
// [TODO] optimize this
static inline unsigned char slab_size_bucket_index(size_t size) {
    if (size <= 32)   return 0;
    if (size <= 64)   return 1;
...
static inline size_t slab_size(size_t size) {
...
void *slab_alloc(size_t size) {
    size = slab_size(size);
    struct slab_bucket *bucket = &slab_buckets[slab_size_bucket_index(size)];

    struct slab_header *header = NULL;
    if (bucket->partial_head != NULL) {
        header = bucket->partial_head;
    } else {
        header = (struct slab_header *)vmm_kernel_mmap(ARCH_VMM_PAGE_SIZE);
        header->slot_size = size;
        header->next_header = NULL;
        header->next_slot_free = NULL;
        header->next_slot_alloc = (char *)((((uintptr_t)((char *)header + sizeof(struct slab_header)) + size - 1) / size) * size);
        bucket->partial_head = header;
    }
    ...
    if (header->next_slot_free != NULL) {
        ...
        if (((uintptr_t)header->next_slot_alloc - (uintptr_t)header) / ARCH_VMM_PAGE_SIZE > 0) {
...
void slab_free(void *addr) {
    struct slab_header *header = (struct slab_header *)(((uintptr_t)addr / ARCH_VMM_PAGE_SIZE) * ARCH_VMM_PAGE_SIZE);

    if (header->next_slot_free == NULL && header->next_slot_alloc == NULL) {
        struct slab_bucket *bucket = &slab_buckets[slab_size_bucket_index(header->slot_size)];
        header_full_to_partial(bucket, header);
    }

    *((char **)addr) = (char *)header->next_slot_free;
    header->next_slot_free = (char *)addr;
}
