...
ipc_endpoint_from_capability(struct capability *capability,
                             enum capability_permission required_permissions) {
    if (!capability_check(capability, CAPABILITY_TYPE_IPC_ENDPOINT,
...
static struct thread *ipc_consume_reply_cap(struct thread *current_thread) {
    struct capability *reply_capability = thread_capability(
        current_thread, CAPABILITY_NODE_RESERVED_SLOT_IPC_REPLY);
    if (!capability_check(reply_capability, CAPABILITY_TYPE_IPC_REPLY,
                          CAPABILITY_PERMISSION_WRITE)) {
        return NULL;
    }
    struct thread *reply_thread =
        (struct thread *)capability_object(reply_capability);
    // capability_burn(thread_capability_node(current_thread),
    // CAPABILITY_NODE_RESERVED_SLOT_IPC_REPLY);
    return reply_thread;
}
...
void syscall_ipc_send(struct syscall_args *args) {
    ...
    struct capability *capability =
        thread_capability(current_thread, args->arg0);
    struct ipc_endpoint *ipc_endpoint =
        ipc_endpoint_from_capability(capability, CAPABILITY_PERMISSION_WRITE);
    ...
    args->ret = SYSCALL_KERROR_SUCCESS;
}
...
void syscall_ipc_receive(struct syscall_args *args) {
    ...
    struct capability *capability =
        thread_capability(current_thread, args->arg0);
    struct ipc_endpoint *ipc_endpoint =
        ipc_endpoint_from_capability(capability, CAPABILITY_PERMISSION_READ);
    if (ipc_endpoint == NULL) {
        args->ret = SYSCALL_ERROR_INVALID_CAPABILITY;
        return;
    }
    ...
    memcpy(message_dest, &current_thread->ipc_context.message,
